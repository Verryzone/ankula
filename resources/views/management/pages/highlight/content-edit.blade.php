...
@section('content')
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500"
                                        aria-current="page">Edit Konten</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                    <h1 class="text-xl lg:text-2xl font-semibold text-gray-900 dark:text-white">Edit Konten</h1>
                    <p class="text-gray-600 dark:text-gray-400 mt-1">Edit konten card "{{ $content->title }}"</p>
                </div>

                <form action="{{ route('management.content.update', $content) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <select name="type" 
                                    ...
                                    <option value="promo" {{ old('type', $content->type) === 'promo' ? 'selected' : '' }}>Promo</option>
                                    <option value="category" {{ old('type', $content->type) === 'category' ? 'selected' : '' }}>Kategori</option>
                                    <option value="featured" {{ old('type', $content->type) === 'featured' ? 'selected' : '' }}>Featured</option>
                                </select>
                                @error('type')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="size" 
                                    ...
                                    <option value="small" {{ old('size', $content->size) === 'small' ? 'selected' : '' }}>Small</option>
                                    <option value="medium" {{ old('size', $content->size) === 'medium' ? 'selected' : '' }}>Medium</option>
                                    <option value="large" {{ old('size', $content->size) === 'large' ? 'selected' : '' }}>Large</option>
                                </select>
                                @error('size')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="text" 
                                       name="price_display" 
                                       id="price_display"
                                       value="{{ old('price_display', $content->price_display) }}" 
                        ...
                        <div class="space-y-6">
                            ...
                            @if($content->image_path)
                                <div>
                                    ...
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <div>
                                            <p class="text-sm text-gray-600">{{ basename($content->image_path) }}</p>
                            ...
                            <div>
                                <label for="image" class="block text-sm font-medium text-gray-700 mb-2">
                                    {{ $content->image_path ? 'Ganti Gambar' : 'Gambar (Opsional)' }}
                                </label>
                                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-blue-400 transition-colors">
                                    <div class="space-y-1 text-center">
                                        ...
                                        <div class="flex text-sm text-gray-600">
                                            <label for="image" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-green-500">
                            ...
                            <div>
                                ...
                                <div class="flex items-center space-x-3">
                                    <input type="color" 
                                           ...
                                           id="background_color"
                                           value="{{ old('background_color', $content->background_color) }}" 
                            ...
                            <div>
                                ...
                                <div class="flex items-center space-x-3">
                                    <input type="color" 
                                           name="text_color" 
                                           id="text_color"
                                           value="{{ old('text_color', $content->text_color) }}" 
                                           class="h-10 w-20 border border-gray-300 rounded cursor-pointer">
                                    <input type="text" 
                                           id="text_color_text"
                                           value="{{ old('text_color', $content->text_color) }}"
                            ...
                            <div id="card-preview" class="border-2 border-gray-200 rounded-lg p-4">
                                ...
                                <div id="preview-card" 
                                    ...
                                    <div class="flex-1 z-10 relative">
                                        <div class="space-y-2">
                                            <h5 id="preview-title" class="font-semibold text-xl drop-shadow-lg">{{ $content->title }}</h5>
                                            <p id="preview-subtitle" class="text-sm opacity-90 drop-shadow-lg">{{ $content->subtitle }}</p>
                                            <div id="preview-price" class="font-bold text-lg drop-shadow-lg">
                                    ...
                                    <div class="w-32 h-32 ml-4 relative z-10 flex-shrink-0">
                                        <div id="preview-image-container" 
                                             ...
                                             @if($content->image_path) 
                                                style="background-image: url('{{ asset('storage/' . $content->image_path) }}'); background-size: cover; background-position: center;"
                                             @endif>
                                            @if(!$content->image_path)
    ...
    <script>
        ...
        function updatePreview() {
            ...
            const subtitle = document.getElementById('subtitle').value || '{{ $content->subtitle }}';
