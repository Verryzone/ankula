    ...
    <div class="pt-4 px-3 mt-2 lg:py-11">
        <div class="max-w-screen-lg mx-auto px-4 sm:px-6 lg:px-8 space-y-6 bg-white rounded-lg shadow mb-56">
            <div class="w-full mb-5">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500"
                                        aria-current="page">Edit Highlight</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                    <h1 class="text-xl lg:text-2xl font-semibold text-gray-900 dark:text-white">Edit Dashboard Highlight</h1>
                    <p class="text-gray-600 dark:text-gray-400 mt-1">Edit highlight yang ditampilkan di dashboard user</p>
                </div>
                ...
                <!-- Form -->
                <form action="{{ route('management.highlight.update', $highlight) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="4"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                    placeholder="Deskripsi singkat tentang highlight ini">{{ old('description', $highlight->description) }}</textarea>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" name="price" id="price" step="0.01" min="0"
                                        ...
                                        placeholder="449.99"
                                        value="{{ old('price', $highlight->price) }}">
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="button_text" id="button_text" required
                                        ...
                                        value="{{ old('button_text', $highlight->button_text) }}">
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="url" name="button_link" id="button_link"
                                        ...
                                        value="{{ old('button_link', $highlight->button_link) }}">
                                </div>
                            </div>
                        </div>
                                        ...
                                        <div id="upload-placeholder">
                                            ...
                                            <div class="flex text-sm text-gray-600">
                                                <label for="image" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                                    <span>{{ $highlight->image_path ? 'Ganti gambar' : 'Upload gambar' }}</span>
                                                </label>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                                    ...
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" name="is_active" id="is_active" value="1" class="sr-only peer" {{ old('is_active', $highlight->is_active) ? 'checked' : '' }}>
                                ...
                                <div>
                                    ...
                                    <input type="number" name="sort_order" id="sort_order" min="0"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                        placeholder="0"
                                        value="{{ old('sort_order', $highlight->sort_order) }}">
                        ...
                        <div id="highlight-preview" class="bg-blue-800 text-white rounded-lg overflow-hidden">
                            <div class="flex flex-col md:flex-row items-center">
                                <div class="p-8 md:p-16 w-full md:w-1/2 space-y-4">
                                    <h1 id="preview-title" class="text-4xl md:text-5xl font-bold leading-tight">{{ $highlight->title }}</h1>
                                    <p id="preview-description" class="text-lg text-blue-200">{{ $highlight->description ?: 'Find the perfect product for your needs.' }}</p>
                                    @if($highlight->price)
                                        <p id="preview-price" class="text-3xl font-bold">Rp {{ number_format($highlight->price, 0, ',', '.') }}</p>
                                    @else
                                        <p id="preview-price" class="text-3xl font-bold" style="display: none;"></p>
                                    @endif
                                ...
                                <div id="preview-image-container" class="relative w-full md:w-1/2 min-h-[30rem] flex items-center justify-center">
                                    @if($highlight->image_path)
                                        <div id="preview-no-image" class="hidden text-center text-blue-200">
                                            <svg class="w-24 h-24 mx-auto mb-4" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm12 12H4l4-8 3 6 2-4 3 6z" clip-rule="evenodd"></path>
                                            </svg>
                                            <p>Gambar belum dipilih</p>
                                        </div>
                                        <img id="preview-image-display" src="{{ asset('storage/' . $highlight->image_path) }}" alt="Preview" class="w-64 md:w-80 object-contain rounded-lg shadow-lg transform rotate-6 transition-transform duration-300 hover:rotate-0">
                                    @else
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updatePreview() {
                previewTitle.textContent = titleInput.value || '{{ $highlight->title }}';
